...
if __name__ == '__main__':
    ...
    rnn1 = RNN('examples/work/data/delay_react/delay_react.pkl', {'dt': 10.0, 'var_rec': 0.01**2})
    trial_args = {}
    info = rnn.run(inputs=(generate_trial, trial_args), seed=200)
    info1 = rnn1.run(inputs=(generate_trial, trial_args), seed=200)
    seed=200
    ...
    plot.plot(rnn.t/tau, rnn1.u[0], color=Figure.colors('green'))
    for i in range(Nout):
        plot.plot(rnn.t/tau, rnn.z[i], color=Figure.colors('red'), label = 'Network Output')
        plot.plot(rnn.t[1:]/tau, Y[i], color =Figure.colors('blue'), label = 'Target Output')
    ...
    plot.legend(['Input', 'Network Output', 'Target Output'])

    fig.save(path='.', name='delay_react_after2ndtask_noewc')
