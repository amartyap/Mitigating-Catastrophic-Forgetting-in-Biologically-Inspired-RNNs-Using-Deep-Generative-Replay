...
r = pd.read_csv('/Users/Amartya/Downloads/mnist-in-csv/mnist_train.csv')
...
class SGD(object):
    ...
    def __init__(self, trainables, inputs, costs, regs, x, z, params, save_values,
        ...
        # Update rule
        '''self.vs = [theanotools.shared_zeros(shape=k.get_value().shape) for k in self.trainables]
        self.ms = [theanotools.shared_zeros(shape=k.get_value().shape) for k in self.trainables]
        self.t = 1
        self.additive = [theanotools.shared_zeros(shape=k.get_value().shape) for k in self.trainables]
        
        momentum = [(m, 0.9 * m + 0.1 * grad) for m, grad in zip(self.ms, g)]
        velocity = [(v, 0.999 * v + 0.001 * (grad**2)) for v, grad in zip(self.vs, g)]
        additive_update1 = [(a, a*0 + lr/(1e-5 + (v/(1-0.999**self.t))**0.5)) for a, v in zip(self.additive,self.vs)]
        additive_update2 = [(a, a*(m/(1-0.9**self.t))) for a, m in zip(self.additive, self.ms)]
        parameter_update = [(theta, theta - a) for theta, a in zip(self.trainables, self.additive)]
        t_update = [(self.t, self.t + 1)]'''
        
        #updates = [(theta, theta - velocity) for theta, velocity in zip(self.trainables, self.vs)]
        #updates = momentum + velocity + additive_update1 + additive_update2 + parameter_update + t_update
        ...
        try:
            ...
            for iter in xrange(first_iter, 1+self.p['max_iter']):
                if iter % checkfreq == 1:
                    ...
                    print('{} updates - {} ({} hrs {} mins {} secs faggot elapsed)'
