...
class Trainer(object):
    ...
    def train(self, savefile, task, recover=True):
        ...
        r  = RNN('examples/work/data/mnist_06/mnist_06.pkl', {'dt': 1, 'var_rec': 0.01**2})
        ...
        #loss = T.sum(f_loss(z[-1,:,:], target[:,:Nout])*mask)/masknorm
        loss = T.sum(f_loss(z, target[:,:,:Nout])*mask)/masknorm
        
        # Root-mean-squared error
        #error = T.sqrt(T.sum(theanotools.L2(z[-1,:,:], target[:,:Nout])*mask)/masknorm) 
